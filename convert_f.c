...
/*   Updated: 2018/01/04 15:58:13 by yazhu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			convert_f(t_format *format, va_list ap, int *ct)
{
	...
	format->min_wd -= (format->precision + ft_digits((unsigned long long)nbr, 10)
			+ show_dot + (sign != '\0'));
	if (fill == '0' && sign)
		ft_putchar(sign);
	...
	if (fill == ' ' && sign)
		ft_putchar(sign);
	*ct += (ft_digits((unsigned long long)nbr, 10) + (sign != '\0'));
